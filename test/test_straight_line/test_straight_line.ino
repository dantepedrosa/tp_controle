 ...
 *
 * Usa:
 *  - interpolacao linear dos ganhos K(PWM) medidos
 *  - feedforward: PWM individual baseado nos modelos
 *  - PI diferencial apenas para correcao fina
 *
 * A logica:
...
float interpK(const MotorModel *table, int N, int pwm)
{
    ...
    return table[N - 1].K;  // fallback (nao deveria ocorrer)
}
